...

    {
        Describe("S3 auto configuration with default storage", () -> {
            ...
            Context("given a default storage type of s3", () -> {
                BeforeEach(() -> System.setProperty("spring.content.storage.type.default", "s3"));
                AfterEach(() -> System.clearProperty("spring.content.storage.type.default"));
                It("should create an S3Client bean", () ->
                        contextRunner.withUserConfiguration(TestConfigWithoutBeans.class)
                                .run((context) ->
                                        Assertions.assertThat(context).hasSingleBean(S3Client.class)));
            });

            Context("given a default storage type other than s3", () -> {
                BeforeEach(() -> System.setProperty("spring.content.storage.type.default", "fs"));
                AfterEach(() -> System.clearProperty("spring.content.storage.type.default"));
                It("should not create an S3Client bean", () -> contextRunner
                        .withUserConfiguration(TestConfigWithoutBeans.class).run((context) ->
                                Assertions.assertThat(context).doesNotHaveBean(S3Client.class)));
            });

            Context("given no default storage type", () -> It("should create an S3Client bean", () ->
                    contextRunner.withUserConfiguration(TestConfigWithoutBeans.class)
                            .run((context) ->
                                    Assertions.assertThat(context).hasSingleBean(S3Client.class))));
        });
