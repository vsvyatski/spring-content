...
public class ContentStoreContentService implements ContentService {
    ...
    public ContentStoreContentService(RestConfiguration config, RepositoryInvoker repoInvoker, MappingContext mappingContext, ContentPropertyToExportedContext exportContext, StoreByteRangeHttpRequestHandler byteRangeRestRequestHandler) {
        this.config = config;
    ...
    public static StoreExportedMethodsMap getExportedMethodsFor(Class<?> storeInterfaceClass, PropertyPath path, ContentPropertyToExportedContext exportContext) {
        ...
        if (exportMap == null) {
            storeExportedMethods.put(key, new StoreExportedMethodsMap(storeInterfaceClass, path, exportContext));
            exportMap = storeExportedMethods.get(key);
        }
    ...
    public static boolean isClientAbortException(Exception e) {
        ...
        if (e.getClass().getSimpleName().equals("ClientAbortException")) {
            ...
            return e.getCause().getClass().getSimpleName().equals("ClientAbortException");
        }
        return false;
    }
    ...
    public void getContent(HttpServletRequest request, HttpServletResponse response, HttpHeaders headers, Resource resource, MediaType resourceType)
        ...
        var storeResource = (AssociatedStoreResource) resource;
    ...
    public void setContent(HttpServletRequest request, HttpServletResponse response, HttpHeaders headers, Resource source, MediaType sourceMimeType, Resource target) throws IOException, MethodNotAllowedException {
        ...
        var storeResource = (AssociatedStoreResource) target;
        ...
        if (methodToUse.getParameters().length > 3 && methodToUse.getParameters()[3].getType().equals(long.class)) {
            ...
            int ordinal = config.getSetContentDisposition().ordinal();
            argsList.add(new SetContentParams(
                    contentLength,
                    true,
                    SetContentParams.ContentDisposition.values()[ordinal]));
        }
    ...
    public void unsetContent(Resource resource) throws MethodNotAllowedException {
        ...
        updateObject = ReflectionUtils.invokeMethod(methodsToUse[0], targetObj, updateObject, storeResource.getPropertyPath(), unsetParams);
    ...
    private Object convertContentArg(Resource resource, Class<?> parameterType) {
        ...
        if (InputStream.class.equals(parameterType)) {
            try {
                ...
                throw new IllegalArgumentException(format("Unable to get input stream from resource %s", resource.getFilename()));
            }
            ...
            if (candidateMimeType != null) {
                ...
                try {
                    ...
                    if (acceptedMimeType.includes(candidateType) && matchParameters(acceptedMimeType, candidateType)) {
                        var store = storeResource.getStoreInfo().getImplementation(ContentStore.class);
    ...
    public static class StoreExportedMethodsMap {
        ...
        static {
            ...
            GET_CONTENT_METHODS = new Method[]{
        ...
        private Method[] calculateExports(Method[] storeMethods, PropertyPath path, ContentPropertyToExportedContext exportContext) {
            ...
            List<Method> exportedMethods = new ArrayList<>(Arrays.asList(storeMethods));
