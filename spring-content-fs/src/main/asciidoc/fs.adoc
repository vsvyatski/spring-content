...
Spring Content File System Stores are enabled with the following Java Config.

.Spring Content File System Stores using Java Config
====
[source, java]
----
@Configuration
...
When using `spring-content-fs-boot-starter` the root for all file system stores will be a randomly chosen folder (at application start-up) under `java.io.tmdir`.
...
File System Storage supports the following signature types:
...
Resources returned by File System Storage are standard Spring Resources implementing the following interfaces:
...
By default, the File System Store Module will store all content at the root of the backing store.  Usually, this
is sufficient.
...
.Spring Content File System Stores customizing storage customization by `@ContentId` type
====
[source, java]
----
@Configuration
...
.Spring Content File System Stores customizing storage customization by Entity type
====
[source, java]
----
@Configuration
